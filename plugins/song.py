...
async def song(client, message):
    ...
    try:
        ...
        print(str(e))
        return
    m.edit("dσwnlσαdíng чσur ѕσng...Boom💣💣💣")
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info_dict = ydl.extract_info(link, download=False)
            audio_file = ydl.prepare_filename(info_dict)
            ydl.process_info(info_dict)
        rep = '[🎥cinema and Song Hυზ | LK Oϝϝιƈιαʅ🎥](https://t.me/cinamahublk)**\n**𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 ›› [DEV](https://t.me/Itzabothmyprofile)'
...
@bot.on_message(filters.command(["video", "mp4"]))
